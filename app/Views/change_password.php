<?php
// Pastikan session dimulai dan data di-load
...
// Logika simulasi pemrosesan form
$message = null;
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['change_password'])) {

  // --- SIMULASI VALIDASI FRONTEND/SECURITY ---
  $old_pass = $_POST['old_password'] ?? '';
  $new_pass = $_POST['new_password'] ?? '';
  $confirm_pass = $_POST['confirm_password'] ?? '';

  if (empty($old_pass) || empty($new_pass) || empty($confirm_pass)) {
    $message = ['type' => 'error', 'text' => 'Semua field wajib diisi.'];
  } elseif ($new_pass !== $confirm_pass) {
    $message = ['type' => 'error', 'text' => 'Kata sandi baru dan konfirmasi tidak cocok.'];
  } elseif (strlen($new_pass) < 6) {
    $message = ['type' => 'error', 'text' => 'Kata sandi minimal 6 karakter.'];
  } else {
    // Di sini seharusnya ada logika untuk memverifikasi old_password di database
    // dan kemudian mengupdate hash kata sandi baru.
    $message = ['type' => 'success', 'text' => 'Kata sandi berhasil diubah! Silakan login kembali.'];
  }
}
?>
...
<body class="bg-gray-50 font-poppins text-gray-800">
  ...
  <div class="max-w-md lg:max-w-2xl mx-auto min-h-screen shadow-xl bg-white lg:rounded-2xl">
    ...
    <header class="flex items-center justify-between p-4 border-b">
      <a href="<?php echo $projectBasePath; ?>profile.php" class="p-2 -ml-2 hover:bg-gray-100 rounded-full transition">
    ...
    <form action="<?php echo $projectBasePath; ?>change_password.php" method="POST" class="p-6 lg:p-10 space-y-6">
